...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const data = await serverLoaders.getLocationBySlug('dubai-marina')
  if (!data) {
...
export default async function DubaiMarinaLocationPage({ params }: { params: Promise<{ locale: string }> }) {
  const data = await serverLoaders.getLocationBySlug('dubai-marina')
