...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const data = await serverLoaders.getIndustryBySlug('financial-services')
  if (!data) {
  ...
  return {
    ...
    alternates: {
      ...
      languages: { en: `${SITE_CONFIG.url}/en/industries/${data.slug}`, ar: `${SITE_CONFIG.url}/ar/industries/${data.slug}` },
    },
  }
}

export default async function FinancialServicesIndustryPage({ params }: { params: Promise<{ locale: string }> }) {
  const data = await serverLoaders.getIndustryBySlug('financial-services')
